defmodule Multigraph.Test.Generators do
  @moduledoc false

  def dag(size) do
    dag(size, Multigraph.new())
  end
  ...
  def biased_dag(size) do
    g = dag(size, Multigraph.new())
    paths = Multigraph.get_paths(g, 1, size)
    ...
    Enum.reduce(shortest, g, fn
      {i, v}, acc ->
        Multigraph.label_vertex(acc, v, path_len - i)
    end)
  end

  def libgraph_to_digraph(%Multigraph{vertices: vs, out_edges: es}) do
    dg = :digraph.new()
